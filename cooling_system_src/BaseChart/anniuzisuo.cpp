...
void AnNiuZiSuo::setAnNiuWH(int w,int h)
{
    if(Type==0)
    {
        Tu.setGeometry(w/20,h/20,w-(w/10),h-(h/10));
        TuXf.setGeometry(0,0,w,h);
        TuDj.setGeometry(w/20,h/20,w-(w/10),h-(h/10));
        TuDjXf.setGeometry(0,0,w,h);
        Tu.setPixmap(pix->scaled(w-(w/10),h-(h/10),Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
        TuXf.setPixmap(pix->scaled(w,h,Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
        TuDj.setPixmap(pixDj->scaled(w-(w/10),h-(h/10),Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
